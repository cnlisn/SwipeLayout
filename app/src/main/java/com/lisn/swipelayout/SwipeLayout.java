...
public class SwipeLayout extends FrameLayout {
    ...
    public void computeScroll() {   //很多次      50+
        ...
        // 确定是否继续触发动画
        boolean b = mViewDragHelper.continueSettling(true);
        if (b) {
            //执行动画
    ...
    public void open(boolean isSmooth) {
        if (isSmooth) {   //平滑的打开   长发公主    scroller
            //   返回决定是否触发动画
            boolean b = mViewDragHelper.smoothSlideViewTo(mFrontLayout, -mRange, 0);
            if (b) {
                // 触发动画   执行动画
                ViewCompat.postInvalidateOnAnimation(this);
            }
    ...
    public void close(boolean isSmooth) {
        if (isSmooth) {
            ...
            boolean b = mViewDragHelper.smoothSlideViewTo(mFrontLayout, 0, 0);
            if (b) {
                //触发动画
                //执行动画
                ViewCompat.postInvalidateOnAnimation(this);
